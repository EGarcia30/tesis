...
<main id="main-content" class="w-custom">
    <div class="w-100 position-relative">
        <div class="d-flex flex-column gap-4 container mx-auto p-3">
            ...
            <div class="card card-height container p-3 bg-white">
                <h1 class="header-font-custom text-utec text-center">Mi Perfil</h1>
                <form action="/tesis/updatePerfil/<?= $this->d['user']->getId()?>" method="post" class="p-2">
                    <div class="mb-3">
                        <label for="" class="form-label text-utec">Nombre:</label>
                        <input type="text"
                        class="form-control" name="nombre" id="nombre" aria-describedby="helpId" value="<?= $this->d['user']->getName() ?>">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text"
                        class="form-control" name="usuario" id="usuario" aria-describedby="helpId" value="<?= $this->d['user']->getUsername() ?>">
                    ...
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="radio" id="inlineRadio1" value="Super Usuario" <?=strpos($this->d['user']->getRol(), 'Super Usuario') !== false ? 'checked' : false ?>>
                        <label class="form-check-label" for="inlineRadio1">Super Usuario</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="radio" id="inlineRadio2" value="Administrador" <?=strpos($this->d['user']->getRol(), 'Administrador') !== false ? 'checked' : false ?>>
                    ...
                    <div class="form-check form-check-inline">
                        ...
                        <label class="form-check-label" for="inlineRadio3" <?=strpos($this->d['user']->getRol(), 'Usuario') !== false ? 'checked' : false ?>>usuario</label>
                    </div>

                    <div class="mt-5">
                        <input type="submit" class="btn btn-utec" value="Actualizar perfil">
    ...
    <!-- Modal -->
    <div class="modal fade" id="staticBackdrop<?= $this->d['user']->getId()?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        ...
        <form action="/tesis/updatePasswordPerfil/<?= $this->d['user']->getId()?>" method="post">
            ...
            <input type="hidden" name="nombre" id="nombre" aria-describedby="helpId" value="<?= $this->d['user']->getName()?>">

            <input type="hidden" name="usuario" id="usuario" aria-describedby="helpId" value="<?= $this->d['user']->getUsername()?>">

            <input type="hidden" name="radio" id="inlineRadio1" aria-describedby="helpId" value="<?= $this->d['user']->getRol()?>">
